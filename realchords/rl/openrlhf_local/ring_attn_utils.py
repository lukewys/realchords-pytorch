...
Contains ring attention utilities from openrlhf.models.ring_attn_utils.

Most of the ring attention utilities are not used in thsi codebase.
"""
...
def convert_ring_attn_params(
    ...
    start, end = (
        ring_attn_rank * local_seq_len,
        (ring_attn_rank + 1) * local_seq_len,
    )
    ...
    if isinstance(sequences, torch.Tensor):
        ...
        attention_mask = torch.cat(
            [
                attention_mask,
                (len(sequences) + 1)
                * torch.ones(1, pad_len, device="cuda", dtype=torch.float),
            ],
            dim=-1,
        )
