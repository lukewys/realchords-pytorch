"""
This file is part of OpenRLHF v0.6.1.post1, copied and modified.
...
class DeepspeedStrategy(ABC):
    ...
    def setup_distributed(self, timeout=timedelta(minutes=60)) -> None:
        ...
        deepspeed.init_distributed(
            timeout=timeout, distributed_port=master_port
        )
                ...
                if self.stage == 3:
                    torch.save(
                        get_peft_model_state_dict(
                            model_to_save, output_state_dict
                        ),
    ...
    def load_ckpt(
        ...
        if load_path is None:
            raise Exception(
                f"[deepspeed] failed to resume from checkpoint {load_dir}"
            )
